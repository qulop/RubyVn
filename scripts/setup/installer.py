...
from setup.utility import download_file, error, success
...
class Installer:
    ...
    def __run_installer(file_path: Path) -> bool:
        ...
        try:
            ...
            error(f"FileNotFoundError: {file}")
            return False
    ...
    def __install_package(os_name: str, target_url: str, win32_package: str, linux_package: str) -> bool:
        ...
        status = Installer.__run_installer(file_path)
        if status:  success("Package successufully installed")
        else:       error("Failed to install package")
