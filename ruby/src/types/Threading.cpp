...
const threadinfo_t
Thread::Priority(threadinfo_t priority) {
	m_priority = priority;
...
const size_t
Thread::Processed() const {
	return m_processed;
}

const size_t
Thread::Processing() const {
	return m_processing;
}
...
bool
Thread::RemoveTask(TaskData task) {
	// Invalid task
	uintptr_t checksum = *reinterpret_cast<uintptr_t*>(&task);
	if (checksum == 0U)
		return false;
...
namespace T {
	Thread*
	Create(threadinfo_t state, threadinfo_t priority) {
